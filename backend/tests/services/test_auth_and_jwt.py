...
async def test_verify_gcp_identity_returns_verified_claims(monkeypatch):
    monkeypatch.setattr(
        auth.auth,
        "verify_id_token",
    ...
    result = await auth.verify_gcp_identity(
...
async def test_verify_gcp_identity_maps_provider_failure(monkeypatch):
    def reject(*_args, **_kwargs):
        raise RuntimeError("bad token")

    monkeypatch.setattr(auth.auth, "verify_id_token", reject)
    with pytest.raises(AuthenticationError):
        await auth.verify_gcp_identity(
            HTTPAuthorizationCredentials(scheme="Bearer", credentials="token")
        )
