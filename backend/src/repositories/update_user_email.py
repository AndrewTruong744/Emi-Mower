...
logger = logging.getLogger("repositories.update_user_email")


async def update_user_email(user_id: str, new_email: str, db: AsyncSession) -> None:
    """
    Updates the email of a user in PostgreSQL using provided AsyncSession.
    ...
    try:
        ...
        logger.error(
            f"Failed to update user email in Postgres: {db_err}", exc_info=True
        )
        await db.rollback()
        raise RepositoryError(f"Failed to update email for user {user_id}") from db_err
